...
export function applySafeAutoChanges(input: {
  ...
  if (current.generatedBy === "us-prison-verifier") {
    return { applied: [], overlayWritten: false };
  }
...
export function emptyUsOverlay(): OverlayFile {
  return { generatedBy: "us-prison-verifier", updatedAt: "", entries: {} };
}

/** Both the CLI --write flag and US_PRISON_VERIFIER_WRITE=1 are required. Default is off. */
export function productionUsWritesEnabled(input: {
  writeFlag?: boolean;
  env?: Record<string, string | undefined>;
}): boolean {
  return Boolean(input.writeFlag) && input.env?.US_PRISON_VERIFIER_WRITE === "1";
}

/**
 * US overlay writes only. Never mutates UK records, BOP import JSON, generated
 * prison modules, prose, titles, or slugs.
 */
export function applyUsSafeAutoChanges(input: {
  countrySlug: string;
  prisonSlug: string;
  sourceUrl: string;
  fields: FieldDiff[];
  ...
  if (!input.writesEnabled) return { applied: [], overlayWritten: false };
  if (isUkPrison({ countrySlug: input.countrySlug }) || !isUsPrison({ countrySlug: input.countrySlug })) {
    return { applied: [], overlayWritten: false };
  }
  ...
  const current = input.store.read() ?? emptyUsOverlay();
  if (current.generatedBy === "uk-prison-verifier") {
    return { applied: [], overlayWritten: false };
  }

  const countrySlug = input.countrySlug === "united-states" ? "united-states" : "us";
  ...
  current.generatedBy = "us-prison-verifier";
  current.entries[key] = entry;
  current.updatedAt = entry.appliedAt;
  input.store.write(current);
  return { applied: safe, overlayWritten: true };
